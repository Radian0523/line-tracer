...
uint16_t sensorMax[8] = {1492, 929, 1310, 1395, 2320, 2187, 2477, 1330};
uint16_t sensorMin[8] = {8, 24, 22, 40, 3, 1, 3, 1};
...
void loop()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    // rawNorm[i] = (float)(raw[i] - sensorMin[i]) / (sensorMax[i] - sensorMin[i]);
    rawNorm[i] = raw[i];
  }
...
float calculateLinePosition()
{
  ...
  {
    // 黒さを計算（0.0〜1.0）
    float darkness = calculateDarkness(rawNorm[i]);
